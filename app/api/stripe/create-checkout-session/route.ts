...
async function lookupStripeCustomerIdByClerkUserId(userId: string): Promise<string | null> {
  const r = await sql`
...
async function customerHasActiveSubscription(stripe: Stripe, customerId: string): Promise<boolean> {
...
export async function POST(req: Request) {
  ...
  if (userId && customer) {
    ...
    if (hasActive) {
      ...
      return NextResponse.json({ok: true, url: portal.url, via: 'billing_portal'})
    }
  }
