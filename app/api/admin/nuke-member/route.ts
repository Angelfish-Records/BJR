...
export async function POST(req: NextRequest) {
  ...
  if (member.clerk_user_id) {
    ...
    if (!clerk.ok) {
      return NextResponse.json({ok: false, error: 'Clerk delete failed', clerk}, {status: 502})
    }
  }
  ...
  try {
    ...
    return NextResponse.json(
      {ok: false, error: 'Neon nuke failed', detail: err instanceof Error ? err.message : String(err)},
      {status: 500}
    )
  }
  ...
  return NextResponse.json({
    ok: true,
    memberFound: true,
    target: {memberId: member.id, emailWas: member.email, clerkUserIdWas: member.clerk_user_id},
