...
export async function GET(req: NextRequest) {
  ...
  if (!slug) {
    return NextResponse.json({ok: false, error: 'Album is missing slug'}, {status: 500})
  }

  const artworkUrl = doc.artwork ? urlFor(doc.artwork).width(900).height(900).quality(85).url() : null
