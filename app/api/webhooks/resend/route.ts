...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const emailId =
      isEmailData && typeof data.email_id === "string" && data.email_id.length
        ? data.email_id
        : null;
    const from =
      isEmailData && typeof data.from === "string" && data.from.length
        ? data.from
        : null;
    ...
    const subject =
      isEmailData && typeof data.subject === "string" && data.subject.length
        ? data.subject
        : null;
