...
function ActionBtn(props: {onClick: () => void; children: React.ReactNode; label: string}) {
  return (
    <button
      type="button"
...
export default function PortalArtistPosts(props: {
  ...
  const components: PortableTextComponents = React.useMemo(
    () => ({
      ...
      block: {
        normal: ({children}) => (
          <p style={{margin: '10px 0', lineHeight: 1.68, fontSize: 13, opacity: 0.92}}>{children}</p>
        ),
        h1: ({children}) => (
            ...
            >
              <div
                style={{
                  ...
                  boxShadow: isDeep ? '0 0 0 2px color-mix(in srgb, var(--accent) 14%, transparent)' : undefined,
                }}
              >
                ...
                <div style={{display: 'flex', gap: 10, alignItems: 'center'}}>
                  ...
                  <div style={{minWidth: 0}}>
                    <div style={{display: 'flex', alignItems: 'baseline', gap: 10, minWidth: 0}}>
                      ...
                      <div style={{fontSize: 12, opacity: 0.56, whiteSpace: 'nowrap'}}>{fmtDate(p.publishedAt)}</div>
                      {p.pinned ? (
                ...
                <div style={{height: 1, background: 'rgba(255,255,255,0.06)', marginTop: 12}} />
              </div>
            </div>
          )
        })}
