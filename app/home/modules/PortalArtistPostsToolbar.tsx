...
type SubmitQuestionCTAProps = Readonly<{
  onOpenComposer: () => void;
}>;
...
type PostTypeMenuProps = Readonly<{
  value: PostTypeFilter;
  onChange: (next: PostTypeFilter) => void;
  constrained?: boolean;
}>;

function asPostTypeFilter(value: string): PostTypeFilter {
  const option = POST_TYPES.find((candidate) => candidate.value === value);
  return option?.value ?? "";
}
  ...
  return (
    ...
    >
      <select
        ...
        value={value}
        onChange={(event) => onChange(asPostTypeFilter(event.target.value))}
        style={{
          height: 30,
          minWidth: 118,
          maxWidth: constrained ? "100%" : undefined,
