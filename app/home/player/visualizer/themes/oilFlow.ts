...
void main() {
  ...
  float viscosity = mix(0.945, 0.80, e);        // slightly “thicker” baseline for inline richness
  float advAmt    = mix(0.010, 0.028, e);
  float diffAmt   = mix(0.012, 0.030, 1.0 - e);
...
void main() {
  ...
  float sheen = smoothstep(0.02, 0.16, gm) * (0.16 + 0.34*e);
  float phase = fract(0.25 + 0.18 * atan(g.y, g.x) + 0.10 * sin(t*0.8) + 0.20 * gm);
  vec3 iri = thinFilm(phase);

  vec3 base = pow(c, vec3(0.86));
  base = mix(vec3(0.025, 0.025, 0.050), base, 1.10);
...
export function createOilFlowTheme(): Theme {
  ...
  return {
    ...
    render(gl, opts) {
      ...
      // IMPORTANT: cap feedback buffer so fullscreen doesn’t explode cost.
      // Inline stays crisp; fullscreen is still lush due to upsampling + diffusion.
      const MAX_FB = 900
      const scale = opts.width > MAX_FB || opts.height > MAX_FB ? 0.75 : 1.0
      const w = Math.max(2, Math.floor(Math.min(MAX_FB, opts.width * scale)))
      const h = Math.max(2, Math.floor(Math.min(MAX_FB, opts.height * scale)))

      if (ping.w !== w || ping.h !== h) ping.resize(gl2, w, h)
