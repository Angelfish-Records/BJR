...
type UploadStatusResponse =
  | {ok: true; status: string; ready: boolean; assetId?: string; playbackId?: string | null}
  | {ok: false; error: string}

export default function MuxUploader(props: {
  onReady: (payload: {uploadId: string; assetId?: string; playbackId: string}) => void
  disabled?: boolean
}) {
  ...
  async function pollUntilReady(id: string): Promise<{assetId?: string; playbackId: string}> {
    for (let i = 0; i < 60; i++) {
      ...
      if (!data.ok) throw new Error(data.error || 'Failed to read upload status')
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      onReady({uploadId: id, assetId: ready.assetId, playbackId: ready.playbackId})
    } catch (e) {
