...
function addUtm(u: URL, method: ShareMethod, targetType: 'album' | 'track' | 'post') {
...
export function buildShareTarget(input:
  ...
  if (input.type === 'post') {
    ...
    const url = origin ? addUtm(new URL(baseAbs), method, 'post').toString() : baseAbs
  ...
  if (input.type === 'album') {
    const url = origin ? addUtm(new URL(baseAbs), method, 'album').toString() : baseAbs
    const title = artist ? `${artist} — ${albumTitle}` : albumTitle
    const text = artist ? `Listen to ${albumTitle} by ${artist}` : `Listen to ${albumTitle}`

    return {type: 'album', albumSlug: input.album.slug, albumId: input.album.id, title, text, url}
  }
  ...
  const title = artist ? `${trackTitle} — ${albumTitle} — ${artist}` : `${trackTitle} — ${albumTitle}`
  const text = artist
...
export function getShareIntents(target: ShareTarget): ShareIntent[] {
  ...
  const intents: ShareIntent[] = [
    ...
    ...(isAndroid()
      ? [{id: 'instagram' as const, label: 'Instagram', href: `intent://#Intent;package=com.instagram.android;scheme=https;end`, note: 'Opens app (paste link)'}]
      : [{id: 'instagram' as const, label: 'Instagram', href: `instagram://app`, note: 'Opens app (paste link)'}]),

    ...(isAndroid()
      ? [{id: 'signal' as const, label: 'Signal', href: `intent://#Intent;package=org.thoughtcrime.securesms;scheme=signal;end`, note: 'Opens app (paste link)'}]
      : [{id: 'signal' as const, label: 'Signal', href: `sgnl://`, note: 'Opens app (paste link)'}]),

    {id: 'telegram', label: 'Telegram', href: `https://t.me/share/url?url=${urlEnc}&text=${textEnc}`},

    {id: 'email', label: 'Email', href: `mailto:?subject=${subj}&body=${body}`},

    {id: 'x', label: 'X', href: `https://twitter.com/intent/tweet?text=${textEnc}&url=${urlEnc}`},

    {
      id: 'messenger',
      label: 'Messenger',
      href: isiOS() ? `fb-messenger://share?link=${urlEnc}` : `https://www.facebook.com/dialog/send?link=${urlEnc}`,
      note: 'May prompt login',
    },

    // Extra: hip hop fans absolutely use Discord + Snapchat.
    {id: 'discord', label: 'Discord', href: `discord://`, note: 'Opens app (paste link)'},

    ...(isAndroid()
      ? [{id: 'snapchat' as const, label: 'Snapchat', href: `intent://#Intent;package=com.snapchat.android;scheme=https;end`, note: 'Opens app (paste link)'}]
      : [{id: 'snapchat' as const, label: 'Snapchat', href: `snapchat://`, note: 'Opens app (paste link)'}]),
  ]

  const seen = new Set<string>()
  return intents.filter((x) => (seen.has(x.id) ? false : (seen.add(x.id), true)))
}
...
export async function performShare(target: ShareTarget): Promise<ShareResult> {
  ...
  if (typeof navigator !== 'undefined') {
    const nav = navigator as Navigator & {
      ...
      canShare?: (data?: {title?: string; text?: string; url?: string; files?: File[]}) => boolean
    }
