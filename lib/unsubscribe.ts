...
export function issueUnsubscribeToken(input: {
  ...
  const now = typeof input.now === 'number' ? input.now : Math.floor(Date.now() / 1000)
  const ttl = Math.max(60, Math.min(60 * 60 * 24 * 30, Math.floor(input.ttlSeconds ?? 60 * 60 * 24 * 14))) // default 14 days
...
export type VerifyResult =
  ...
  | {ok: false; error: 'MISSING' | 'MALFORMED' | 'BAD_SIG' | 'EXPIRED' | 'INVALID_PAYLOAD'}

export function verifyUnsubscribeToken(token: string, now?: number): VerifyResult {
  const t = token.trim()
  ...
  const decoded = safeJsonParse<UnsubscribeTokenPayload>(base64UrlDecode(body).toString('utf8'))
  if (!decoded || decoded.v !== 1) return {ok: false, error: 'INVALID_PAYLOAD'}

  if (typeof decoded.email !== 'string' || !decoded.email.trim()) return {ok: false, error: 'INVALID_PAYLOAD'}
  if (typeof decoded.iat !== 'number' || typeof decoded.exp !== 'number') return {ok: false, error: 'INVALID_PAYLOAD'}

  const nowSec = typeof now === 'number' ? now : Math.floor(Date.now() / 1000)
  if (decoded.exp < nowSec) return {ok: false, error: 'EXPIRED'}

  return {ok: true, payload: {...decoded, email: normalizeEmail(decoded.email)}}
}
...
export function maskEmail(email: string): string {
  ...
  const domMasked = domMain.length <= 2 ? '***' : domMain[0] + '***' + domMain.slice(-1)
